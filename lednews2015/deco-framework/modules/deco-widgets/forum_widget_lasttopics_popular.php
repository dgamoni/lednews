...
class forum_widget_lasttopics_popular_Widget extends WP_Widget {

	/** constructor */
	function __construct() {

		parent::WP_Widget( false, $name = __( 'Форум - популярное', TEXTDOMAIN ) );
	}
	...
	function widget( $args, $instance ) {
		...
		echo '<div class="widget forum-widget">';
			...
			echo do_shortcode('[lasttopics_popular count="'.$posts_num.'" views="0"]');
	...
	function form( $instance ) {
		...
		$title     = esc_attr( $instance['title'] ) ? esc_attr( $instance['title'] ) : 'Популярное';
...
add_action( 'widgets_init', create_function( '', 'return register_widget("forum_widget_lasttopics_popular_Widget");' ) );
