...
function deco_wsl_addon_login_process_register_js() {
	wp_enqueue_style( 'deco-wsl-addon-login-process', DECO_LOGIN_URL . 'assets/css/deco-wsl-addon-login-process.css', array(), DECO_LOGIN_STYLE_SCRIPT_VER );

	wp_enqueue_script( "deco-wsl-addon-login-process", DECO_LOGIN_URL . "assets/js/deco-wsl-addon-login-process.js", array(), DECO_LOGIN_STYLE_SCRIPT_VER, true );
}
...
function deco_wsl_addon_login_process_popup_soc_auth_message() {
	if ( isset( $_GET['popup_auth'] ) && preg_match( '/wordpress_social_authenticate/', $_SERVER['HTTP_REFERER'], $match ) ) {
		include DECO_LOGIN_PATH . 'page-wsl-auth-for-popup.php';
		die();
	}
}
...
function deco_wsl_addon_login_process_popup() {
	include_once DECO_LOGIN_PATH . 'popups.php';
}
