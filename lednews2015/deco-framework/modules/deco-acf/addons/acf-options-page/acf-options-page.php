	...
	*/
	...
	function __construct() {
		...
		if ( ! defined( 'OPTIONS_NAME' ) ) {
			define( 'OPTIONS_NAME', 'Настройка темы' );
		}
		// vars
		$this->settings = array(
			'title'      => __( OPTIONS_NAME, 'acf' ), // title / menu name ('Site Options')
	...
	*/
	...
	function admin_menu() {
		...
		if ( ! empty( $this->settings['pages'] ) ) {
			$parent_title = $this->settings['pages'][0]['title'];
			$parent_slug  = 'acf-options-' . $this->settings['pages'][0]['slug'];
		}
		...
		if ( ! empty( $this->settings['pages'] ) ) {
			foreach ( $this->settings['pages'] as $page ) {
				...
				$sub_slug  = 'acf-options-' . $page['slug'];
		...
		<script type="text/javascript">
			(function ($) {
				...
				$(window).scroll(function () {
					var option_page_block_save = $('.inner-sidebar .postbox .acf-button').parent().parent();
					var offset = option_page_block_save.offset();

					console.log(offset.top + " = " + $(window).scrollTop());

					console.log($(window).scrollTop() + ' - ' + offset.top);
//					if ($(window).scrollTop() >= offset.top) {
						option_page_block_save.css({
							position : 'fixed',
							top      : '32px',
							'margin-top': '139px'
						});
...
//					} else {
//						option_page_block_save.removeAttr('style');
//					}
				});
				...
				<form id="post" method="post" name="post">
					<div class="metabox-holder has-right-sidebar" id="poststuff">
						...
						<div class="inner-sidebar" id="side-info-column">
							...
							<!-- Update -->
							<div class="postbox">
								<h3 class="hndle"><span><?php _e( "Сохранение настроек", 'acf' ); ?></span></h3>
...
if ( ! function_exists( 'register_options_page' ) ) {
	...
	function register_options_page( $title = "", $slug = "" ) {
		if ( empty( $slug ) ) {
			echo 'Empty slug option page!!!';

			return;
		}

		$array_pages                    = array( 'title' => $title, 'slug' => $slug );
		$GLOBALS['acf_options_pages'][] = $array_pages;
	}
