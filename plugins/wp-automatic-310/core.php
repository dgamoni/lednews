...
class wp_automatic {
	...
	function process_campaign($camp) {
		...
		if(in_array('OPT_LIMIT_TITLE', $camp_opt)){
		...
		if (trim ( $title ) != '') {
			...
			if (in_array ( 'OPT_THUMB', $camp_opt )) {
				...
				if (count ( $srcs ) > 0) {
					...
					$this->log ( 'Featured image', '<a href="' . $src . '">' . $src . '</a>' );
					
					echo '<br>Featured image src: '.$src;
			...
			if($camp_type == 'Youtube'   ){
				...
				if(in_array('OPT_YT_COMMENT', $camp_opt) ){
					...
					$comments_link="http://gdata.youtube.com/feeds/api/videos/$vid_id/comments?v=2&alt=json";
					...
					curl_setopt($this->ch, CURLOPT_URL, trim($url));
				 	$exec=curl_exec($this->ch);
				 	$exec=str_replace('$t', 'x', $exec);
				 	$exec=str_replace('yt$userId', 'ytid', $exec);
					...
					if(trim($exec) !=''){
						...
						if( stristr($exec, 'entry')){
							$comments_array=json_decode($exec);
							
							$entry=$comments_array->feed->entry ;
							...
							if(count($entry) == 0){
								...
								foreach($entry as $comment ){
									...
									$commentText= $comment->content->x;
									$commentAuthor= $comment->author[0]->name->x;
									$commentAuthorID= $comment->author[0]->uri->x;
									$commentUri= str_replace('http://gdata.youtube.com/feeds/api/users/','http://www.youtube.com/user/',$commentAuthorID);
			...
			//TrueMag integration 
			if( $camp_type == 'Youtube' && null !==  PARENT_THEME   && PARENT_THEME =='truemag'){
				echo '<br>TrueMag theme exists adabting config..';
			...
			}elseif ($camp_type == 'Youtube' && null !==  PARENT_THEME   && PARENT_THEME =='truemag'){
				...
				$key = $key [0];
					...
					if (count ( $links ) != 0) {
						...
						foreach ( $links as $link ) {
							...
							$author_link = 'http://ezinearticles' . $matches [1] [0];
		...
		foreach ( $keywords as $keyword ) {
			...
			if (trim ( $keyword ) != '') {
				...
				if (count ( $res ) > 0) {
					...
					$chart_url = "http://www.amazon.com/gp/aws/cart/add.html?AssociateTag=$tag&ASIN.1=$asin&Quantity.1=1&SubscriptionId=$subscription";
					...
					if(trim($ret->link_price) != ''){
						...
						preg_match('{\d.*\d}is', ($offer_price),$price_matches);
	...
	function feed_process_link($feed, $camp) {
		...
		// fix dgamoni
		$my_meta_value = $item->get_item_tags('', 'my_meta_value');
			...
			if (! $this->is_duplicate($url) ) {
				...
				$res ['tags'] = '';
				// fix dgamoni
				$res ['enclosure_link'] = $my_meta_value[0]["data"];
				...
				if (in_array ( 'OPT_FULL_FEED', $camp_opt )) {
					...
					if ($result) {
						...
						if(trim($valid) != ''){
							...
							preg_match_all('{og:image.*?content="(.*?)"}s', $html,$matches);
							
							$og_imgs=$matches[1];
							
							if(isset($og_imgs[0])){
								 
								 $res ['og_img']=$og_imgs[0];
							}		
				...
				if(in_array('OPT_FEED_ENTITIES', $camp_opt)){
					...
					$res ['matched_content'] = html_entity_decode($html);
				...
				if( in_array ( 'OPT_MUST_IMAGE', $camp_opt ) &&   ! stristr($res['cont'], '<img')     ) {
					...
					preg_match_all('{<img.*?src[\s]*=[\s]*["|\'](.*?)["|\'].*?>}s', $res['cont'] , $matches);
	...
	function fb_get_post($camp){
		...
		//building feed
		if(trim($cg_fb_page_id)){
			$cg_fb_page_feed = "https://www.facebook.com/feeds/page.php?format=rss20&id=$cg_fb_page_id";
			echo '<br>Processing Feed:'.$cg_fb_page_feed;
			
			include_once (ABSPATH . WPINC . '/feed.php');
			
			// Get a SimplePie feed object from the specified feed source.
			$rss = fetch_feed ( $cg_fb_page_feed );
			...
			if (! is_wp_error ( $rss )){ // Checks that the object is created correctly
				...
				if (!isset($maxitems) || $maxitems == 0) return false;
				...
				foreach ( $rss_items as $item ){
					...
					if(trim($title) == '' && in_array('OPT_GENERATE_FB_TITLE', $camp_opt) ){
						...
						$newTitle =  substr(strip_tags($content), 0,80) ;
					...
					//if video 
					if(stristr($url, 'video.php')){
						 
						//remove style
						$content = str_replace('style="height:90px"', '', $content);
						
						//add video shortcode
						$vi_parts = explode('v=', $url);
						$vid_id = $vi_parts[1];
					...
					if(stristr($content, 's130x130')){
						echo '<br>Small images found extracting full images..';
						
						preg_match_all('{"https://.*?s130x130/(.*?)\..*?"}', $content,$matches);
						...
						if(stristr($exec, $small_imgs_ids[0])){
							...
							foreach ($small_imgs_ids as $small_imgs_id){
								...
								if(trim($large_imgs_matches[1]) != ''){
									$content = str_replace( $small_imgs_srcs[$i], $large_imgs_matches[1], $content);
								} 
	...
	function youtube_fetch_links($keyword, $camp) {
		...
		if(! in_array('OPT_YT_FULL', $camp_opt) && !in_array ( 'OPT_YT_PLAYLIST', $camp_opt ) ){
		...
		foreach ( $search as $itm ) {
			...
			*/
			...
			if ( ! $this->is_duplicate($link_url) )  {
				$query = "INSERT INTO {$this->wp_prefix}automatic_youtube_links ( link_url , link_title , link_keyword  , link_status , link_desc ,link_time,link_rating ,link_views,link_player,link_img,link_author,link_duration) VALUES ( '$link_url', '$link_title', '{$camp->camp_id}_$keyword', '0' ,'$link_desc','$link_time','$link_rating','$link_views','$link_player','$link_img','$author','$link_duration')";
				$this->db->query ( $query );
			} else {
				echo ' <- duplicated <a href="'.get_edit_post_link($this->duplicate_id).'">#'.$this->duplicate_id.'</a>';
			}
		...
		foreach ( $keywords as $keyword ) {
			...
			if (trim ( $keyword ) != '') {
				...
				if (count ( $res ) > 0) {
					...
					$temp ['vid_player'] =  '<iframe width="'.$width.'" height="'.$height.'" src="//www.youtube.com/embed/'.$vid_id.'" frameborder="0" allowfullscreen></iframe>';
				...
				if (count ( $res ) > 0) {
					...
					if( trim($temp['pin_title']) == '' ){
						...
						if(in_array('OPT_PT_AUTO_TITLE', $camp_opt)){
							...
							$newTitle = substr($temp['pin_description'], 0,$cg_pt_title_count);
	...
	function gtranslate($title, $content, $from, $to) {
		...
		// match links
		// $regexp = "<a\s[^>]*href=(\"??)([^\" >]*?)\\1[^>]*>(.*)<\/a>";
		preg_match_all ( "/<a\s[^>]*href=(\"??)([^\" >]*?)\\1[^>]*>(.*)<\/a>/s", $text, $matches, PREG_PATTERN_ORDER );
		$htmlurls = $matches [0];
		$htmlurls = array ();
		...
		$htmlfounds = array_merge ( $htmlurls, $htmlfounds );
		$htmlfounds[] = '&quot;'; 
		...
		$translated = '';
		foreach ( $sentences as $sentense ) {
			@$translated .= $sentense->trans;
		}
		...
		if (stristr ( $translated, '***********' )) {
			...
			//fx added spaces [199900 ]  or [ 199000] or [1,999,oo ]
			$translated = preg_replace('{\[\s*([\d,]+)\s*\]}', '[$1]', $translated);
			
			//fix added , with numbers like 199,ooo,pla pla pla
			preg_match_all('{\[[\d|,]*\]}', $translated,$brackets);
