<?php
	...
	if ( ! defined( 'ABSPATH' ) || class_exists( 'Decomments_Plugin_Updater' ) ) {
		return;
	}
	...
	class DESTAT_Updater {
		...
		public function disable_wporg_request( $r, $url ) {
			...
			if ( isset( $r_body['plugins'] ) ) {
				...
				} /* if unserialize didn't work ( POST WP.3.7 using json ) */ else {
		...
		public function updater_data() {
			...
			$file_path = DECOM_PLUGIN_PATH . '/' . $updater_data['file'];
			...
			if ( true === $this->config['username'] && true === $this->config['dashboard'] ) {
				...
				$widget_option = get_site_option( $widget_id );
		...
		public function transient_update_plugins( $checked_data ) {
			...
			$remote_url     = add_query_arg( array(
				                                 'plugin_repo' => $updater_data['repo_slug'],
				                                 'ahpr_check'  => $updater_data['version']
			                                 ), $updater_data['repo_uri'] );
			$remote_request = array(
				'timeout'    => 20,
				'body'       => array(
					'key'        => $updater_data['key'],
					'login'      => $updater_data['login'],
					'autohosted' => $updater_data['autohosted']
				),
				'user-agent' => 'WordPress/' . $wp_version . '; ' . $updater_data['domain']
			);
		...
		public function plugins_api_result( $res, $action, $args ) {
			...
			if ( ! isset( $args->slug ) ) {
				...
				} /* If it's not on plugin info iframe (e.g. update core page) */ else {
			...
			if ( $plugin_slug == $updater_data['slug'] && $action == 'plugin_information' ) {
				...
				$remote_url     = add_query_arg( array(
					                                 'plugin_repo' => $updater_data['repo_slug'],
					                                 'ahpr_info'   => $updater_data['version']
				                                 ), $updater_data['repo_uri'] );
				$remote_request = array(
					'timeout'    => 20,
					'body'       => array(
				...
				if ( is_wp_error( $request ) ) {
				...
				} /* If no error, construct the data */ else {
					...
					} /* If data is empty or not an object */ else {
		...
		public function add_dashboard_widget() {
			...
			if ( current_user_can( 'update_plugins' ) ) {
				...
				wp_add_dashboard_widget( $widget_id, $widget_name, array(
					&$this,
		...
		public function dashboard_widget_callback() {
			...
			if ( ! empty( $widget_option ) && is_array( $widget_option ) ) {
				...
				} /* if key is not valid */ elseif ( $widget_option['status'] == 'invalid' ) {
			...
			} /* if activation key is not yet set/empty */ else {
		...
		public function dashboard_widget_control_callback() {
			...
			if ( isset( $_POST[ $widget_id ] ) && isset( $_POST['dashboard-widget-nonce'] ) && wp_verify_nonce( $_POST['dashboard-widget-nonce'], 'edit-dashboard-widget_' . $widget_id ) ) {
				...
				if ( ! empty( $response ) ) {
					...
					if ( $response == 'valid' ) {
						$valid = 'valid';
					} /* if key is not valid */ elseif ( $response == 'invalid' ) {
						$valid = 'invalid';
					} /* if response is value is not recognized */ else {
						$valid = 'unrecognized';
					}
				} /* if response is empty or error */ else {
					$valid = 'error';
				}
				...
				/* save value */
				update_site_option( $widget_id, $input );
			}
